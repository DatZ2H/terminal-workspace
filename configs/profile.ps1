...
function Update-Tools {
    ...
    $packages = @("JanDeDobbeleer.OhMyPosh", "Git.Git", "OpenJS.NodeJS.LTS")
...
function Get-Status {
    ...
    $tools = @(
        ...
        @{ Name = "PSReadLine";     Cmd = { (Get-Module PSReadLine).Version.ToString() } }
        @{ Name = "Terminal-Icons"; Cmd = { (Get-Module Terminal-Icons -ListAvailable | Select-Object -First 1).Version.ToString() } }
...
$GuideClaudeDir = "$env:USERPROFILE\Claude\Cowork\PNX-Vault\Guide Claude"
function gc-doc { Set-Location $GuideClaudeDir; claude }
function gs-doc { Set-Location $GuideClaudeDir; git status }
function gl-doc { Set-Location $GuideClaudeDir; git log --oneline -10 }
function gd-doc { Set-Location $GuideClaudeDir; git diff --stat }
...
function update-claude {
    ...
    if (Select-String -Path $cliJs -Pattern "Vietnamese IME fix" -Quiet) {
        ...
        python "$env:USERPROFILE\.claude-vn-fix\patcher.py" --path $cliJs
    }
}
